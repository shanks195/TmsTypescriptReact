...
export interface SelectNameTextRef{
  getValue(): IVersion | undefined;
}

export interface SelectNameTextProps{
  className?: string;
  label?: string;
  placeholder?: string;
  message?: string;
...
const SelectNameText: ForwardRefRenderFunction<SelectNameTextRef, SelectNameTextProps> = (props, ref) => {
  ...
  const options = Version.map(c => ({ value: c.id, label: c.name }));
...
export default forwardRef(SelectNameText);